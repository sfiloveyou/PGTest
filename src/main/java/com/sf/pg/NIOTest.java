...
//			}).start();
    ...
    protected static void serverHandler(SelectionKey key) throws IOException {
    	if (key.isAcceptable()) {
    		...
    		//for (int i = 0; i < PROXY_HOST.length; i++) {
    			int r= RANDOM.nextInt(2);
    			System.out.println(r);
				String client = PROXY_HOST[r];
	    		SocketChannel clientChannel = SocketChannel.open();  
	    		clientChannel.configureBlocking(false);
	    		clientChannel.connect(new InetSocketAddress(client, PROXY_PORT));
	    		MyChannel mychannel = new MyChannel();
	            mychannel.setClient(client);
	            mychannel.setSocketChannel(clientChannel);
	            mychannel.setAuthed(false);
	            channel.register(selector, SelectionKey.OP_READ,mychannel);
        	...
        	SocketChannel in = (SocketChannel) key.channel();
        	SocketChannel out= null;
        	MyChannel myChannel = null;
        	if(key.attachment() instanceof  MyChannel){
        		myChannel = (MyChannel) key.attachment();
        		out = myChannel.getSocketChannel();
        	}else{
        		out = (SocketChannel) key.attachment();
        	}
